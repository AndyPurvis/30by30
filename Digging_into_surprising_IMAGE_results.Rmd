...
Looking into the BID IMAGE data, there's a huge disjunction in the managed forest beetween 2020 and 2025, with 100 Mha being lost. If that's being lost in the areas where endemic richness then fell, I have a likely explanation for what's gone on.
...
new_ena_BID <- convert_iam_data(file.path(in_stem, BID_file), which_bii, file.path(ena_stem, "Checking_BID_ENA.nc"), file.path(pdf_stem, "Checking_BID.pdf"), layers=c(1:19))

CRJ_file <- "BendingTheCurve30by30-LCproj-IMAGE-RCPref_SSP2_CRJ-06Feb2020_FinalMask.nc"
new_ena_CRJ <- convert_iam_data(file.path(in_stem, CRJ_file), which_bii, file.path(ena_stem, "Checking_CRJ_ENA.nc"), file.path(pdf_stem, "Checking_CRJ.pdf"), layers=c(1:19))

HPR_file <- "BendingTheCurve30by30-LCproj-IMAGE-RCPref_SSP2_HPR-06Feb2020_FinalMask.nc"
new_ena_HPR <- convert_iam_data(file.path(in_stem, HPR_file), which_bii, file.path(ena_stem, "Checking_HPR_ENA.nc"), file.path(pdf_stem, "Checking_HPR.pdf"), layers=c(1:19))
