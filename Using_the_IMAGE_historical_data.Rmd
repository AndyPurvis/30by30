...
this.k <- 0.0122
this.z <- 0.269
S_t_hist <- relax(natarea=historic_annual_area, years=c(850:2020), wanted=c(850:2020), rsr_2015=rsr_array, k=this.k, z=this.z)
...
abline(v=1970, col="red", lty=2)
extinctions_map(S=S_t_hist, year=c(850:2020), between=c(850,2020), main_stem="Historical")
...
for (i in 1:length(image_files)){
  ...
  aslu_array <- as.array(aslu)
  saveRDS(aslu_array, file.path("output/arrays/", paste(image_files[i], ".Rds", sep="")))

  # relax using aslu array
  S_t_scen <- relax(natarea=aslu_array, years=c(2010:2100), wanted=c(2010:2100), rsr_2015=rsr_array, k=this.k, z=this.z)
  ...
  percentage_plot(A=aslu_array, S=S_t_scen, year=c(2010:2100), main_stem=shortnames[i])
  ...
  cat(paste(shortnames[i], "2010-2100: extinction summary\n"))
  print(scen_totals)
}
...
The results are most naturally expressed as cumulative numbers of extinctions over time, on a relative scale. The GDN scenario is projected to prevent more extinctions than any other, with HPR (surprisingly?) second and NRS (unsurprisingly) last. GDN prevents just over 30% of the extinction projected under the baseline scenario (NRS).
...
text(2020, 65, adj=0, "HPR", col="green")
text(2020, 60, adj=0, "GDN", col="yellow")
